...
function renderClientList(data) {
    // Group clients by their group field
    const groupedClients = {};
    const ungroupedClients = [];
    
    data.forEach(function(obj) {
      if (obj.Client.group && obj.Client.group.trim() !== '') {
        const groupName = obj.Client.group.trim();
        if (!groupedClients[groupName]) {
          groupedClients[groupName] = [];
        }
        groupedClients[groupName].push(obj);
      } else {
        ungroupedClients.push(obj);
      }
    });
    ...
    groupNames.forEach(function(groupName) {
      ...
      // Render clients in this group
      groupedClients[groupName].forEach(function(obj) {
        $('#client-list').append(renderClientCard(obj));
      });
    });
    
    // Render ungrouped clients
    if (ungroupedClients.length > 0) {
      if (groupNames.length > 0) {
        const ungroupedHeader = `
          <div class="col-12">
            <h5 class="mt-4 mb-2"><i class="fas fa-folder"></i> Ungrouped</h5>
          </div>
        `;
        $('#client-list').append(ungroupedHeader);
      }
      ungroupedClients.forEach(function(obj) {
        $('#client-list').append(renderClientCard(obj));
      });
    }
  }
  ...
  /**
   * Renders a single client card.
   * @param {Object} obj - Client data object.
   * @returns {string} - The HTML for the client card.
   */
  function renderClientCard(obj) {
